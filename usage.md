...
追加したデバイス(上記の例では/dev/sda2)はほぼ空の状態なので、ストレージプールを構成するデバイス間にデータを均等に配分するのが望ましいです。そのために、次に述べるバランス処理を実施します。
