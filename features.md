...
- ストレージプール: 複数のデバイス(あるいはパーティション。以下単に区別せず、デバイスと記載)から、一つの大きなストレージプールを作成できます
...
従来型のファイルシステムは、1つのファイルシステムを1つのデバイス上に作成します。これに対してBtrfsのファイルシステムは、1つだけではなく複数のデバイスから作成できます。既存のBtrfsファイルシステムに対するデバイスの追加、削除、交換も可能です。
